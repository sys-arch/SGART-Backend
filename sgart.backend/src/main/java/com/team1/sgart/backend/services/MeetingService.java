...
public class MeetingService {
	...
	public boolean cancelMeetingIfAllInvitationsRejected(UUID meetingId, UUID excludedInvitationId) {
		...
		// Recuperamos todas las invitaciones EXCEPTO la que se está procesando por si
		// no se ha guardado Rechazada aún
		List<Invitations> invitations = invitationsDao.findByMeetingIdAndInvitationIdNot(meetingId,
		...
		if (allRejected) {
			// ------------------------------------------------------------------------------------Esto
			// es igual que cancelMeetingByOrganizer se podría juntar :P
			Optional<Meetings> meetingOpt = meetingDao.findById(meetingId);
			if (meetingOpt.isEmpty()) {
				throw new RuntimeException("Reunión no encontrada");
			}
