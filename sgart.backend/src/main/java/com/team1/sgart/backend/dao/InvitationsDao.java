...
public interface InvitationsDao extends JpaRepository<Invitations, Integer> {
	...
	int countByMeetingIdAndUserId(@Param("meetingId") UUID meetingId, @Param("userId") UUID userId);

	@Modifying(flushAutomatically = true, clearAutomatically = true)
	@Transactional
	@Query(value = "UPDATE SGART_InvitationsTable " + "SET INVITATION_STATUS = :newStatus, REJECTION_REASON = :comment "
			+ "WHERE MEETING_ID = :meetingId AND USER_ID = :userId", nativeQuery = true)
	int updateInvitationStatus(@Param("meetingId") UUID meetingId, @Param("userId") UUID userId,
			@Param("newStatus") String newStatus, @Param("comment") String comment);
